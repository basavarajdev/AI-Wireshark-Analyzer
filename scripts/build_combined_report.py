...
# ── Channel RF / performance data (from channel survey analysis) ──────────────
RF = {
    1:  {"util":42.5,"retry":16.4,"ctrl":35.8,"rts":27.2,"nav":32573,"bssids_rf":33,"clients_rf":479,
         "status":"High retry + Control overhead + Hidden node","status_cls":"bad"},
    2:  {"util":25.7,"retry":0.6,"ctrl":3.3,"rts":0.3,"nav":32567,"bssids_rf":25,"clients_rf":398,
         "status":"OK","status_cls":"ok"},
    3:  {"util":29.7,"retry":24.9,"ctrl":43.3,"rts":28.6,"nav":25566,"bssids_rf":29,"clients_rf":418,
         "status":"High retry + Control overhead + Hidden node","status_cls":"bad"},
    4:  {"util":11.2,"retry":0.3,"ctrl":3.5,"rts":0.1,"nav":32667,"bssids_rf":22,"clients_rf":364,
         "status":"OK","status_cls":"ok"},
    5:  {"util":8.1,"retry":3.4,"ctrl":11.2,"rts":7.9,"nav":30000,"bssids_rf":30,"clients_rf":381,
         "status":"OK","status_cls":"ok"},
    6:  {"util":37.1,"retry":5.3,"ctrl":8.8,"rts":8.0,"nav":32767,"bssids_rf":91,"clients_rf":471,
         "status":"Busiest channel · Max NAV","status_cls":"warn"},
    7:  {"util":13.7,"retry":81.1,"ctrl":7.5,"rts":4.0,"nav":32341,"bssids_rf":27,"clients_rf":325,
         "status":"High retry (severe)","status_cls":"bad"},
    8:  {"util":7.3,"retry":68.7,"ctrl":7.1,"rts":4.3,"nav":2710,"bssids_rf":29,"clients_rf":311,
         "status":"High retry","status_cls":"bad"},
    9:  {"util":11.2,"retry":49.8,"ctrl":4.3,"rts":5.2,"nav":30000,"bssids_rf":29,"clients_rf":326,
         "status":"High retry","status_cls":"bad"},
    10: {"util":23.6,"retry":95.9,"ctrl":8.1,"rts":4.3,"nav":12878,"bssids_rf":27,"clients_rf":244,
         "status":"High retry (extreme)","status_cls":"bad"},
    11: {"util":37.4,"retry":17.4,"ctrl":8.9,"rts":19.1,"nav":31000,"bssids_rf":37,"clients_rf":539,
         "status":"High retry + Hidden node","status_cls":"bad"},
    12: {"util":21.0,"retry":8.2,"ctrl":3.2,"rts":60.6,"nav":1726,"bssids_rf":23,"clients_rf":385,
         "status":"Hidden node (extreme RTS)","status_cls":"warn"},
    13: {"util":8.4,"retry":0.0,"ctrl":0.9,"rts":0.0,"nav":11305,"bssids_rf":17,"clients_rf":237,
         "status":"Low probe response rate","status_cls":"ok"},
}

# ── Client map data ────────────────────────────────────────────────────────────
data = json.loads(Path("results/channel_survey2/client_network_map.json").read_text())
...
<div class="hero">
  ...
  <div class="meta">2.4 GHz Band · Channels 1–13 · Generated {gen_time} · All 13 channels surveyed</div>
</div>

<div class="toc">
  {''.join(f'<a href="#ch{c}">CH {c}</a>' for c in range(1,14))}
</div>

<!-- Summary cards -->
<div class="cards">
  <div class="card"><div class="card-label">Channels Surveyed</div><div class="card-val" style="color:var(--blue)">13</div></div>
  <div class="card"><div class="card-label">Total APs</div><div class="card-val" style="color:var(--blue)">{sum(CM[c].get('aps',0) for c in range(1,14))}</div></div>
  <div class="card"><div class="card-label">Confirmed Clients</div><div class="card-val" style="color:var(--ok)">{sum(CM[c].get('total',0)-CM[c].get('aps',0) for c in range(1,14))}</div></div>
  <div class="card"><div class="card-label">Channels Healthy</div><div class="card-val" style="color:var(--ok)">{sum(1 for c in range(1,14) if health_cls(score(c))=='ok')}</div></div>
  <div class="card"><div class="card-label">Channels Degraded</div><div class="card-val" style="color:var(--warn)">{sum(1 for c in range(1,14) if health_cls(score(c))=='warn')}</div></div>
  <div class="card"><div class="card-label">Channels Critical</div><div class="card-val" style="color:var(--bad)">{sum(1 for c in range(1,14) if health_cls(score(c))=='bad')}</div></div>
  <div class="card"><div class="card-label">Peak Retry CH</div><div class="card-val" style="color:var(--bad)">CH 10 <span style="font-size:1rem">95.9%</span></div></div>
  <div class="card"><div class="card-label">Busiest Channel</div><div class="card-val" style="color:var(--warn)">CH 6 <span style="font-size:1rem">37.1%</span></div></div>
</div>
...
<div style="padding:16px 40px 40px;color:var(--sub);font-size:12px">
  Capture source: 2.4 GHz passive channel survey · Channels 1–13<br>
...
out = Path("results/channel_survey2/comprehensive_network_report.html")
out.write_text(HTML)
