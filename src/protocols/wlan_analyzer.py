...
class WLANAnalyzer:
    ...
    def _calculate_statistics(self, df: pd.DataFrame) -> Dict:
        ...
        sas = df['sa'].dropna()
        sas_real = sas[sas.apply(lambda x: is_globally_administered(x.lower()))]
        stats['unique_transmitters'] = int(sas_real.nunique())  # real OUI only
        stats['unique_transmitters_all'] = int(sas.nunique())  # including virtual
        stats['top_transmitters'] = sas.value_counts().head(10).to_dict()

        das = df['da'].dropna()
        das_real = das[das.apply(lambda x: is_globally_administered(x.lower()) and is_unicast(x.lower()))]
        stats['unique_receivers'] = int(das_real.nunique())  # real OUI unicast only
        stats['unique_receivers_all'] = int(das[das.apply(is_unicast)].nunique())  # all unicast
        ...
        # RTS / CTS overhead
        rts_count = int((df['type_subtype'] == '0x001b').sum())
        cts_count = int((df['type_subtype'] == '0x001c').sum())
        total_frames = len(df)
        stats['rts_frames'] = rts_count
        stats['cts_frames'] = cts_count
        stats['rts_cts_overhead_pct'] = float(round((rts_count + cts_count) / max(total_frames, 1) * 100, 2))
    ...
    def _detect_threats(self, df: pd.DataFrame) -> Dict:
        ...
        # RF / medium-access checks delegated to WLANRFMonitor
        rf_monitor = WLANRFMonitor(self.wlan_config)
        for key, finding in rf_monitor.run_all(df).items():
            threats[key] = finding
